...
const getConfig = () => {
  const configPath = join(import.meta.dir, '../../../deployment/.localnet/op-stack.json')
  ...
  if (existsSync(configPath)) {
    ...
    return {
      ...
      l1ChainId: parseInt(process.env.L1_CHAIN_ID || String(config.l1ChainId)),
      l2ChainId: parseInt(process.env.L2_CHAIN_ID || String(config.l2ChainId)),
      optimismPortal: (process.env.OPTIMISM_PORTAL || config.contracts?.OptimismPortal) as Address,
    }
  }
  ...
  return {
    ...
    l1ChainId: parseInt(process.env.L1_CHAIN_ID || '3151908'),
    l2ChainId: parseInt(process.env.L2_CHAIN_ID || '901'),
...
describe('Real L1 → L2 Derivation', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log('   bun run packages/deployment/scripts/start-real-op-stack.ts')
      shouldSkip = true
      return
    }

    // Check for OptimismPortal
    if (!config.optimismPortal || config.optimismPortal === '0x0000000000000000000000000000000000000000') {
      console.log('⚠️ OptimismPortal address not configured. Skipping deposit tests.')
      shouldSkip = true
      return
    }

    // Check portal exists
    const portalCode = await l1Client.getCode({ address: config.optimismPortal })
    ...
    l1WalletClient = createWalletClient({
      chain: { id: config.l1ChainId, name: 'L1', nativeCurrency: { name: 'ETH', symbol: 'ETH', decimals: 18 }, rpcUrls: { default: { http: [config.l1Rpc] } } },
      transport: http(config.l1Rpc),
      account: FUNDER,
    })
...
`)
  ...
  it('should deposit ETH from L1 and see it on L2 (REAL DERIVATION)', async () => {
    if (shouldSkip) {
    ...
    const l2BalanceBefore = await l2Client.getBalance({ address: RECIPIENT.address })
    ...
    console.log(`   Waiting for L2 derivation (max ${DERIVATION_TIMEOUT / 1000}s)...`)
    ...
    while (Date.now() - startTime < DERIVATION_TIMEOUT) {
      ...
      if (currentL2Block > l2BlockBefore) {
        l2BalanceAfter = await l2Client.getBalance({ address: RECIPIENT.address })
        
        // Check if balance increased
        if (l2BalanceAfter > l2BalanceBefore) {
          derived = true
          console.log(`   L2 derived at block ${currentL2Block}`)
          console.log(`   L2 balance after: ${formatEther(l2BalanceAfter)} ETH`)
          break
        }
      }
    ...
    if (!derived) {
      ...
      console.log(`   This may indicate op-node is not properly deriving from L1`)
    }
