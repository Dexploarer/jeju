...
class SecureSecretsService {
  ...
  async getSecrets(): Promise<PlatformSecrets> {
    // Check cache
    if (this.cachedSecrets && Date.now() - this.lastFetch < this.cacheTtlMs) {
  ...
  private async fetchFromKMS(): Promise<PlatformSecrets> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[Secrets] KMS request failed: ${response.status}`)
      ...
      if (isProductionEnv() && !parsed.data.decryptedInTee) {
        console.warn('[Secrets] Warning: Secrets were not decrypted in TEE.')
      }
