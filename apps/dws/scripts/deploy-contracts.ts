...
async function startAnvil(): Promise<void> {
  ...
  const proc = Bun.spawn(['anvil', '--port', '6546', '--chain-id', '31337'], {
    stdout: 'pipe',
    stderr: 'pipe',
  })
...
async function getContractBytecode(contractName: string): Promise<{ abi: readonly object[]; bytecode: Hex }> {
  ...
  for (const path of possiblePaths) {
    if (existsSync(path)) {
      const artifact = await Bun.file(path).json() as { abi: readonly object[]; bytecode: { object: string } }
      return {
        abi: artifact.abi,
...
const SIMPLE_REGISTRY_BYTECODE = '' as Hex
...
async function updateContractsConfig(deployment: DeploymentResult): Promise<void> {
...
async function saveDeploymentInfo(deployment: DeploymentResult): Promise<void> {
  ...
  await Bun.write(deploymentPath, JSON.stringify({
    network: 'localnet',
    chainId: 31337,
    rpcUrl: RPC_URL,
...
export async function deployDWSContracts(): Promise<DeploymentResult> {
  ...
  const privateKey = (process.env.DEPLOYER_PRIVATE_KEY || DEFAULT_PRIVATE_KEY) as Hex
  ...
  let cacheManager: Address
  let cronOrchestrator: Address
  let containerRegistry: Address
  let managedDatabaseRegistry: Address
  
  try {
    const gitContract = await deployContract(walletClient, publicClient, 'RepoRegistry', [account.address, '0x0000000000000000000000000000000000000000'])
    gitRegistry = gitContract.address
  } catch {
    gitRegistry = await deploySimpleRegistry(walletClient, publicClient, 'GitRegistry')
  }
  
  try {
    const pkgContract = await deployContract(walletClient, publicClient, 'PackageRegistry', [account.address])
    packageRegistry = pkgContract.address
  } catch {
    packageRegistry = await deploySimpleRegistry(walletClient, publicClient, 'PackageRegistry')
  }
  
  // These are simpler contracts that may not exist yet
  cacheManager = await deploySimpleRegistry(walletClient, publicClient, 'CacheManager')
  cronOrchestrator = await deploySimpleRegistry(walletClient, publicClient, 'CronOrchestrator')
  containerRegistry = await deploySimpleRegistry(walletClient, publicClient, 'ContainerRegistry')
  managedDatabaseRegistry = await deploySimpleRegistry(walletClient, publicClient, 'ManagedDatabaseRegistry')
