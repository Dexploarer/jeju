...
function generateDeploymentUrl(subdomain: string, _target: DeploymentTarget): string {
...
async function processDeployment(deployment: Deployment): Promise<void> {
  try {
    ...
    addBuildLog(deployment, `Build completed in ${Math.round(deployment.buildDuration / 1000)}s`)
...
export function createGitHubIntegrationRouter() {
  return new Elysia({ prefix: '/deploy' })
    ...
    .post('/create', async ({ body }) => {
      const { projectId, name, target, framework: _framework, regions, meta } = body as {
        projectId: string
        name: string
        target: DeploymentTarget
        framework: string
      ...
      const subdomain = target === 'preview' 
        ? `${meta?.gitBranch ?? 'preview'}-${name}`.toLowerCase().replace(/[^a-z0-9-]/g, '-')
        : name
    ...
    .get('/:id/logs', async function* ({ params, query }) {
      ...
      while (true) {
        ...
        if (!follow || deployment.state === 'READY' || deployment.state === 'ERROR') {
          break
        }
    ...
    .post('/:id/promote', async ({ params }) => {
      ...
      const prodDeployment: Deployment = {
        ...
        url: generateDeploymentUrl(deployment.subdomain.replace(/^pr-\d+-/, ''), 'production'),
        createdAt: new Date(),
        updatedAt: new Date(),
    ...
    .delete('/preview/:subdomain', async ({ params }) => {
      ...
      for (const [id, deployment] of deployments) {
        if (deployment.subdomain === subdomain && deployment.target === 'preview') {
    ...
    .get('/projects/:projectId/deployments', ({ params, query }) => {
      ...
      const projectDeploymentIds = deploymentsByProject.get(params.projectId) ?? []
    ...
    .post('/projects/:projectId/env', async ({ params, body }) => {
      ...
      project.environmentVariables = project.environmentVariables.filter(e => e.key !== key)
      ...
      project.environmentVariables.push({
        ...
        value,
        target,
        encrypted: key.includes('SECRET') || key.includes('KEY') || key.includes('TOKEN'),
      })
    ...
    .delete('/projects/:projectId/env/:key', async ({ params }) => {
      ...
      project.environmentVariables = project.environmentVariables.filter(e => e.key !== params.key)
      project.updatedAt = new Date()
    ...
    .post('/projects/:projectId/domains', async ({ params, body }) => {
      ...
      return {
        ...
        verification: {
          records: [
            { type: 'TXT', name: verification.name, value: verification.value },
            { type: 'CNAME', name: domain, value: `${project.id}.dws.jejunetwork.org` },
          ],
        },
      }
    })
    ...
    .delete('/projects/:projectId/domains/:domain', async ({ params }) => {
      ...
      project.domains = project.domains.filter(d => d.domain !== params.domain)
      project.updatedAt = new Date()
      ...
      switch (event) {
        case 'push': {
          ...
          if (branch === 'main' || branch === 'master') {
            ...
            console.log('[GitHub Webhook] Push to main - triggering production deploy')
          }
          break
        }
        ...
        case 'pull_request': {
          ...
          const prBranch = (pr?.head as Record<string, unknown>)?.ref ?? 'unknown'

          if (action === 'opened' || action === 'synchronize') {
            // Trigger preview deployment
            console.log(`[GitHub Webhook] PR #${prNumber} (${prBranch}) updated - triggering preview deploy`)
          } else if (action === 'closed') {
            // Cleanup preview deployment
            console.log(`[GitHub Webhook] PR #${prNumber} closed - cleaning up preview`)
          }
          break
        }
