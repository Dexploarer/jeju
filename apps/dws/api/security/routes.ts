...
import { getAccessControl, CreateRoleSchema, CreateAPIKeySchema } from './access-control'
import { getSecretsManager, CreateSecretSchema } from './secrets-manager'
...
export function createSecurityRoutes() {
  ...
  return new Elysia({ prefix: '/security' })
    ...
    .group('/waf', (app) =>
      app
        ...
        // Add WAF rule
        .post('/rules', async ({ body, headers }) => {
          const owner = headers['x-wallet-address'] as Address
          if (!owner) {
            return { error: 'Unauthorized' }
          }

          const rule = waf.addRule(body as Parameters<typeof waf.addRule>[0])
        ...
        // Block IP
        .post('/block-ip', async ({ body, headers }) => {
          const owner = headers['x-wallet-address'] as Address
          if (!owner) {
            return { error: 'Unauthorized' }
          }
        ...
        // Unblock IP
        .post('/unblock-ip', async ({ body, headers }) => {
          const owner = headers['x-wallet-address'] as Address
          if (!owner) {
            return { error: 'Unauthorized' }
          }
    ...
    .group('/access', (app) =>
      app
        ...
        // Assign role to user
        .post('/users/:userId/roles', async ({ params, body, headers }) => {
          const owner = headers['x-wallet-address'] as Address
          if (!owner) {
            return { error: 'Unauthorized' }
          }
        ...
        .delete('/users/:userId/roles/:roleId', async ({ params, headers }) => {
          const owner = headers['x-wallet-address'] as Address
          if (!owner) {
            return { error: 'Unauthorized' }
          }
        ...
        .post('/check', async ({ body, headers }) => {
          ...
          const decision = accessControl.checkAccess(
            user.userId,
            body.resource as Parameters<typeof accessControl.checkAccess>[1],
        ...
        // Create API key
        .post('/api-keys', async ({ body, headers }) => {
          const address = headers['x-wallet-address'] as Address
          if (!address) {
            return { error: 'Unauthorized' }
          }
          ...
          const { key, apiKey } = accessControl.createAPIKey(user.userId, params)
          ...
          body: t.Object({
            ...
            resourceFilter: t.Optional(t.Object({
              type: t.String(),
        ...
        .get('/organizations', ({ headers }) => {
          ...
          const organizations = accessControl.getUserOrganizations(user.userId)
          return { organizations }
        })

        // Create organization
        .post('/organizations', async ({ body, headers }) => {
          const address = headers['x-wallet-address'] as Address
          if (!address) {
            return { error: 'Unauthorized' }
          }

          const org = accessControl.createOrganization(body.name, body.slug, address)
    ...
    .group('/secrets', (app) =>
      app
        ...
        .get('/:secretId', async ({ params, headers }) => {
          ...
          const value = await secretsManager.getSecret(params.secretId, address)
          if (!value) {
        ...
        .post('/:secretId/rotate', async ({ params, headers }) => {
          ...
          const secret = await secretsManager.rotateSecret(params.secretId, address)
          if (!secret) {
            return { error: 'Secret not found or unauthorized' }
          }
        ...
        .delete('/:secretId', async ({ params, headers }) => {
          ...
          const success = await secretsManager.deleteSecret(params.secretId, address)
        ...
        .get('/env/:scopeId', async ({ params, headers }) => {
          ...
          const env = await secretsManager.getEnvironmentSecrets(params.scopeId, address)
          return { env }
        })
    )
    ...
    .group('/audit', (app) =>
      app
        ...
        .get('/logs', ({ query, headers }) => {
          ...
          const decision = accessControl.checkAccess(user.userId, 'audit', 'read')
          if (!decision.allowed) {
            return { error: 'Forbidden' }
          }

          const q = query ?? {}
          type CategoryType = Parameters<typeof auditLogger.query>[0]['category']
          type OutcomeType = Parameters<typeof auditLogger.query>[0]['outcome']
          const result = auditLogger.query({
            ...
            startTime: q.startTime ? parseInt(q.startTime as string, 10) : undefined,
            endTime: q.endTime ? parseInt(q.endTime as string, 10) : undefined,
        ...
        .post('/reports', async ({ body, headers }) => {
          const address = headers['x-wallet-address'] as Address
          if (!address) {
            return { error: 'Unauthorized' }
          }
          ...
          const report = auditLogger.generateComplianceReport(
            body.type as Parameters<typeof auditLogger.generateComplianceReport>[0],
            body.startTime,
            body.endTime,
          )
        ...
        .get('/export', ({ query, headers }) => {
          ...
          const data = auditLogger.export({
            startTime: query.startTime ? parseInt(query.startTime as string, 10) : undefined,
