...
test.describe('Swap Page - Core UI', () => {
  ...
  test('renders swap page with header', async ({ page }) => {
    ...
    await expect(page.getByText(/Swap tokens or bridge across chains/i)).toBeVisible()
  })
  ...
  test('displays chain selectors', async ({ page }) => {
    ...
    const chainSelects = page.locator('select').filter({ has: page.locator('option', { hasText: /Jeju|Ethereum|Base/i }) })
  ...
  test('displays You Receive section', async ({ page }) => {
    ...
    // Output is read-only
    const outputAmount = page.locator('input').filter({ hasText: /0\.0/ }).or(page.locator('input[readonly]'))
  ...
  test('has token selectors', async ({ page }) => {
    ...
    const tokenSelects = page.locator('select').filter({ has: page.locator('option', { hasText: 'ETH' }) })
...
test.describe('Swap - Token Selection', () => {
  ...
  test('lists available tokens', async ({ page }) => {
    ...
    let foundTokenSelect = false
    for (let i = 0; i < count; i++) {
      const select = allSelects.nth(i)
      const options = await select.locator('option').allTextContents()
      console.log(`Select ${i} options:`, options)
      if (options.includes('ETH')) {
        foundTokenSelect = true
      }
    }
...
test.describe('Swap - Chain Selection', () => {
  ...
  test('can select source chain', async ({ page }) => {
    ...
    const sourceChainSelect = page.locator('#source-chain').or(
      page.locator('select').filter({ has: page.locator('option', { hasText: 'Jeju' }) }).first()
    )
    
    if (await sourceChainSelect.isVisible()) {
      const options = await sourceChainSelect.locator('option').allTextContents()
  ...
  test('can select destination chain', async ({ page }) => {
    ...
    const destChainSelect = page.locator('#dest-chain').or(
      page.locator('select').filter({ has: page.locator('option', { hasText: 'Jeju' }) }).nth(1)
    )
  ...
  test('shows warning when cross-chain unavailable', async ({ page }) => {
    ...
    const chainSelects = page.locator('select').filter({ has: page.locator('option', { hasText: 'Jeju' }) })
...
test.describe('Swap - Amount Input', () => {
  ...
  test('clears output when input cleared', async ({ page }) => {
    ...
    expect(outputValue === '' || outputValue === '0.0' || outputValue === '0').toBe(true)
  })
})
...
test.describe('Swap - Button States', () => {
  ...
  test('shows "Connect Wallet" when disconnected', async ({ page }) => {
    ...
    const enterAmountButton = page.getByRole('button', { name: /Enter Amount/i })
  ...
  test('shows appropriate button text when no amount entered', async ({ page }) => {
    await assertNoPageErrors(page)
  ...
  test('swap button is disabled when no wallet connected', async ({ page }) => {
    ...
    const swapButton = page.getByRole('button', { name: /Connect Wallet/i }).last()
