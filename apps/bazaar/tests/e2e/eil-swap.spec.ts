...
test.describe('EIL Cross-Chain - Chain Selection', () => {
  ...
  test('lists supported chains', async ({ page }) => {
    ...
    if (await chainSelect.isVisible()) {
      ...
      const hasOtherChains = expectedChains.some(chain => options.includes(chain))
      console.log('Has cross-chain support:', hasOtherChains)
    }
  })

  test('can select different source and destination chains', async ({ page }) => {
    await assertNoPageErrors(page)
    ...
    if (await chainSelects.count() >= 2) {
      ...
      if (destOptions.includes('Ethereum')) {
        ...
        expect(await destSelect.inputValue()).not.toBe(await sourceSelect.inputValue())
      }
    }
  })
...
test.describe('EIL Cross-Chain - Fee Display', () => {
  ...
  test('shows fee estimate for cross-chain swap', async ({ page }) => {
    ...
    if (await chainSelects.count() >= 2) {
      ...
      const destOptions = await chainSelects.nth(1).locator('option').allTextContents()
      if (destOptions.includes('Ethereum')) {
  ...
  test('shows warning when cross-chain bridge unavailable', async ({ page }) => {
    ...
    if (await chainSelects.count() >= 2) {
      ...
      if (destOptions.includes('Ethereum')) {
        ...
        const warningText = page.getByText(/bridge not available|cross-chain.*unavailable/i)
